...
async function backupRecovery(env, op) {
    ...
    const backupOrRecoveryScriptSecretName = await createBackupRecoveryScriptSecret({
        namespace: namespace,
        op: op,
    })
    const overrides = await _getOverride(
        backupOrRecoveryScriptSecretName,
        tnsAdminSecretName,
    ...
    return childProcess('oc', [
        ...
        return childProcess('oc', ['-n', `${namespace}`, 'delete', `secret/${backupOrRecoveryScriptSecretName}`])
    })
} // end backup
