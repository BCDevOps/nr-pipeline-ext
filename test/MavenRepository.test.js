...
describe("MavenRepository", function() {
   ...
   context("cache", function() {
      it("fetch from public repository", function() {
         ...
         return repo.cache({groupId:'org.liquibase', artifactId:'liquibase-core', version:'3.5.3'}).then( (file)=>{
      ...
      it("fetch from private repository", function() {
         ...
         return repo.cache({groupId:'org.liquibase', artifactId:'liquibase-core', version:'3.5.3', ext:'pom'}).then( (file)=>{
            expect(file).not.toBeUndefined();
            expect(file).not.toBeNull();
            expect(fs.existsSync(file)).toBe(true);
         })
