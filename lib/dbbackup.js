...
(async function () {
    ...
    const tnsAdminSecretName = config.phases.dev.backup.tnsAdminSecretName;
    const dbBackupCredentialSecretName = config.phases.dev.backup.dbBackupCredentialSecretName;

    await backup(env);

    /**
     * Create backup container on Openshift to do db backup for a specified 'env'
     * It uses '--image=overridden' option to override template to create pod.
     * 
     * @param {*} env environment to backup
     */
    ...
    /**
     * Create and save temporary backup script secrert on Openshift for db backup.
     * Secrete name is randomly generated so don't run into conflict for multiple pipeline.
     * Assume there is a template script at ./lib/backup.sql
     * @param {*} options 
     */
    function createBackupScriptSecret(options) {
        ...
        const backupScriptFilePath = path.join(__dirname, "backup.sql");
     ...
     * @param {*} backupScriptSecretName backup script secret name generated on Openshift.
