...
(async function () {
    ...
    async function backup(env) {
        ...
        const backupScriptSecretName = await createBackupScript({namespace: namespace});
        ...
        const backupPodName = "sqlcl-" + Math.random().toString(36).substring(2, 6);
    ...
    function createBackupScript(options) {
        const secretId = Math.random().toString(36).substring(2, 10)
        const secretName = `backup-script-${secretId}`
        const backupScriptFileName = "backup-script.sql";
        const tempFilePath = path.join(tempDir, backupScriptFileName);

        if (!fs.existsSync(tempDir)){
            fs.mkdirSync(path.dirname(tempDir), {recursive:true});
        }

        const backupFilestream = fs.createWriteStream(tempFilePath);
        backupFilestream.write(Buffer.from("SELECT sysdate FROM dual;\n"));
        backupFilestream.write(Buffer.from("EXIT\n"));
        ...
        return childProcess('oc', [
            ...
            'generic',
            secretName,
            `--from-file=backup-script.sql=${tempFilePath}`
        ]).then( code =>{
            return secretName;
        });
    }
