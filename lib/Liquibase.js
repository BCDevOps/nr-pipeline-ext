...
class Liquibase {
  ...
  async liquibaseMigrate(defaultsFile, liquibaseJar, ojdbcJar, url, schemaName){ 
  ...
  await replace({files: this.settings.stages['recovery'].changelog, from: /id=\"[0-9]*\"/g, to:'id=\"'+tDate+'\"'});
   ...
   async migrate (migrationsDir, schemaName){
    this.install().then( (artifacts) =>{
      ...
      return this.liquibaseMigrate(propertiesFilePath, artifacts.liquibase, artifacts.drivers[0], url, schemaName)
    })
  }
}
