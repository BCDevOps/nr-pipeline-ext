...
module.exports = class Liquibase {
    constructor(settings) {
        ...
        this.liquibaseDownloadSite =
            'https://repo1.maven.org/maven2/org/liquibase/liquibase-core/3.8.0/liquibase-core-3.8.0-bin.tar.gz'
    ...
    downloadDrivers() {
        ...
        if (this.jdbcDrivers) {
            const mavenRepository = new MavenRepository(
                'https://bwa.nrs.gov.bc.ca/int/artifactory/libs-release',
                ...
                if (file.exists === true) {
                    ...
                    return new Promise(resolve => {
                        ...
                        spawn('curl', [
                            ...
                            '-o',
                            file.fileName,
                            self.liquibaseDownloadSite,
                            '--output',
                            self.liquibaseHomeDir,
        ...
        return this.downloadDrivers().then(drivers => {
            ...
            return new Promise(resolve => {
                ...
                const changeLogFile = path.relative(
                    migrationsDir,
                    path.join(migrationDir, 'changelog', schemaName + '.xml')
                )
    ...
    migrate(migrationDir, schemaName) {
        const migrationsDir = path.dirname(migrationDir)
        return this._prepare(migrationDir, schemaName).then(args => {
            return this.run([`--url=${args.url}`, `--defaultsFile=${args.propertiesFilePath}`, 'migrate'], {
                cwd: migrationsDir,
            }).then(result => {
    ...
    async backup() {
        ...
        await replace({
            files: this.settings.stages.recovery.changelog,
            from: /id="[0-9]*"/g,
            to: 'id="' + tDate + '"',
        })
    }

    status(migrationDir, schemaName) {
        const migrationsDir = path.dirname(migrationDir)
        return this._prepare(migrationDir, schemaName).then(args => {
            return this.run([`--url=${args.url}`, `--defaultsFile=${args.propertiesFilePath}`, 'status', '--verbose'], {
                cwd: migrationsDir,
            }).then(result => {
                // clean up anything sensitive
                return new Promise(resolve => {
    ...
    rollback(migrationDir, schemaName, count) {
        const migrationsDir = path.dirname(migrationDir)
        return this._prepare(migrationDir, schemaName).then(args => {
            return this.run(
                [
                    `--url=${args.url}`,
                    `--defaultsFile=${args.propertiesFilePath}`,
                    '--verbose',
                    'rollbackCount',
                    `${count}`,
                ],
