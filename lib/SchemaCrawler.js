...
class SchemaCrawler {
    constructor(url, user, password, jdbcDrivers, mavenCredentails) {
        ...
        this.schemaCrawlerDownloadSite =
            'https://github.com/schemacrawler/SchemaCrawler/releases/download/v15.06.01/schemacrawler-15.06.01-distribution.zip'
        ...
        return Promise.resolve(self.schemaCrawlerZipFile)
            ...
            .then(file => {
                if (file.exists === true) {
                    ...
                    return new Promise(resolve => {
                        spawn('curl', ['-sSL', '-o', file.fileName, self.schemaCrawlerDownloadSite]).on(
                            'close',
            ...
            .then(homeDir => {
                return this.downloadDrivers().then(() => {
                    return `${homeDir}/schemacrawler-15.06.01-distribution/_schemacrawler`
                })
            })
    }
     ...
     */
    version() {
        return this.run(['-version']).then(proc => {
    ...
    run(args) {
        ...
        return this.install().then(homeDir => {
