...
(async function () {
  ...
  const url = "".concat(routeSpce.tls.termination === 'edge'? 'https' : 'http', '://', routeSpce.host, routeSpce.path);
  console.log(`Retrieving url as: '${url}' for environment: ${targetEnv}`);

  // current phase settings
  const uitestConfig = {
      srcFolderName: 'functional-testing', // default test folder name, relative to project folder
      skip: 'false', // default
      runCommand: "../npmw run headless-chrome-test", // sample command
      env: {'TARGET_ENV': targetEnv, 'TARGET_TESTING_URL': url}
  };
