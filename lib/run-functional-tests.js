...
(async function () {
  ...
  const uitestConfig = {
      ...
      // env: Object.assign(process.env, {'TARGET_ENV': targetEnv, 'TARGET_TESTING_URL': targetUrl})
      env: Object.assign({}, {PATH:process.env.PATH, HOME:process.env.HOME}, {'TARGET_ENV': targetEnv, 'TARGET_TESTING_URL': targetUrl})
  };
  ...
  console.log("Inherit process.env in script:", process.env);
  console.log("Process environment: ", uitestConfig.env);
  try {
  ...
  // childProcess('npm', ['ci'], {cwd: testFolderDir})
  //   .then(() => {
  //     console.log("npm ci executed successfully.")
  //     return childProcess('npm', ['run', 'headless-chrome-tests'], {cwd: testFolderDir, env: uitestConfig.env});
  //   })
  //   .then((result) => {
