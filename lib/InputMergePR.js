...
module.exports = class {
  ...
  async verifyBeforeDeployment () {
    ...
    if (Array.from(mapRfdIssueStatus.values()).every((val, arr) => val === 'Closed') && rfcStatus.toLowerCase() === 'closed') {
      status = 'Ready'
    } else {
      status = 'Not Ready'
    }

    for (const i of mapRfdIssueStatus.keys()) {
      console.log('\n---------------------------     CURRENT RFD PROD STATUS     ---------------------------\n')
      if (mapRfdIssueStatus.get(i).toLowerCase() !== 'closed') {
        console.log('=> Status of RFD:', i, 'is currently ', mapRfdIssueStatus.get(i), '; ACTION REQUIRED: Close to continue merge')
      } else {
        console.log('=> RFD:', i, ' is closed')
      }
    }

    console.log('\n---------------------------   RFC STATUS     ---------------------------\n')
    if ((rfcStatus.toLowerCase() !== 'closed')) {
      console.log('=> Current RFC Status is', rfcStatus, ', ACTION REQUIRED: Close RFC', rfcIssueKey, 'to continue Merge')
    } else {
