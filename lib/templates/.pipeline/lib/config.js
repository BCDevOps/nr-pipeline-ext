...
const openjdkImageTag = '1.6-23'
const openjdkImageName = 'openjdk18-openshift'
const tomcatImageTag = '9.0.28'
const tomcatImageName = 'ha-tomcat9'
const webappName = ''
const artifactId = ''
const packagingType = ''
...
const phases0 = {
  ...
  tag:           {build: `build-${version}-${changeId}`, dev: `dev-${version}-${changeId}`                                           , dlvr: `dlvr-${version}`                                                , test: `test-${version}`                                           , prod: `prod-${version}`},
  instance:      {build: `${name}-build-${changeId}`   , dev: `${name}-dev-${changeId}`                                              , dlvr: `${name}-dlvr`                                                   , test: `${name}-test`                                              , prod: `${name}-prod-${changeId}`},
  transient:     {build: true                          , dev: true                                                                   , dlvr: false                                                            , test: false                                                       , prod: false},
  host:          {build: ''                            , dev: `${name}-dev-${changeId}-${namespace.dev}.pathfinder.gov.bc.ca`        , dlvr: `${name}-dlvr-${namespace.dev}.pathfinder.bcgov`                 , test: `${name}-test-${namespace.test}.pathfinder.bcgov`           , prod: `a100.gov.bc.ca`},
};
...
module.exports = exports = {phases, options, jiraUrl, bitbucketUrl, openjdkImageTag,openjdkImageName, tomcatImageTag, tomcatImageName, webappName, artifactId, packagingType, artifactVersion };