...
module.exports = class Git {
   ...
   // Get latest Commit
    getLatestCommitOnTarget(username,password,gitUrl,changeTarget){
       var whichGit=gitUrl.split('\/')[2]
       if(whichGit==="bwa.nrs/gov.bc.ca"){
   ...
   return this.run(['ls-remote', gitUrl, `refs/heads/${changeTarget}`]).then((output) =>  {
      return new Promise( (resolve, reject) =>{
         var commitHash=output.stdout.split('\t')[0]
         resolve(commitHash)
      });
      ...
      return this.getMergeBase(changeBranch,changeTarget).then((commitHash1) =>  {
         return this.getLatestCommitOnTarget(user,pass,url,changeTarget).then((commitHash2) =>{
            return new Promise( (resolve) =>{
               console.log(`MergeBase - commitHash1: ${commitHash1}`);
               console.log(`LatestCommitOnTarget - commitHash2: ${commitHash2}`);
