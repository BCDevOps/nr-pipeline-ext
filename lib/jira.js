'use strict';
const JiraClient = require('jira-connector'); // Use npm module jira-connector
...
class Jira {       // class defintion for the class jira
  ...
  async createRFD() {
    var jira = this.createJiraClient();
    ...
    var rfcIssueVersion = rfcIssue.fields.fixVersions[0].name
    //console.log(rfcIssue)
    var arraySize = rfcIssueLinks.length;
    //console.log(arraySize)
    var rfcIssueKey = this.settings.jira.rfcIssueKey;
    var rfcIssueStatus =  rfcIssue.fields.status.name;
    ...
    if (arraySize == 0) {
      //console.log("In IF")
      const project_info= await jira.project.getProject({
        projectIdOrKey: projectName
        , function(error, issue) {
        }
      });
      var component_info = project_info.components
      var component='undefined'
      await component_info.forEach(async function(element){
        if(element.name == repoName){
        component= element.name
        }
      });
       if(component == 'undefined'){
        await jira.component.createComponent({
          component: {
          name: `${repoName}`,
          project: `${projectName}`
          }
      ...
      var environment = [ 'DLVR', 'TEST', 'PROD' ]
      const self = this;
      ...
      const rfdSubtaskIssue1 = await jira.issue.createIssue({
        ...
        fields: {
          ...
          summary: `RFD-Subtask-${env}-${changeBranch}-${branchName}-${reviewer}-Review`,
          customfield_10121: {
            value: `${env}`,
          },
          components: [{ name: `${repoName}` }]
