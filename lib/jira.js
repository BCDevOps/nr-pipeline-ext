...
class Jira {
   ...
   async createRFD() {
      ...
      if (!containsIssueLinks) {
         ...
         await REVIEWER_ENV_MAP.forEach(async function(reviewer, env) {
            // new RFD created.
   ...
   async _manageRfdAndSubtasksTransitionToInitialState(rfdIssueId) {
      ...
      if (rfdIssueStatus == ISSUE_STATUS_NAME.SUBMITTED) {
         ...
         //re-submit
         await this.transition(rfdIssueId, ISSUE_TRANSITION_ACTION_NAME.RESUBMIT);
      }else if (
         ...
         /* await this.transition(rfdIssueId, ISSUE_TRANSITION_ACTION_NAME.CANCEL);
         try{
            await this.transition(rfdIssueId, ISSUE_TRANSITION_ACTION_NAME.REOPEN_ISSUE);
            }
            catch(err){
