...
function ask(){
    ...
    .prompt([
        ...
        const dbChange=answers.typeOfObject + '_' + answers.nameOfObject.replace(/[ #$.&*-]/g,'_')
        const retVal= generate.execute(answers.typeOfMigration.toString(),answers.schemaname,answers.username,dbChange.toUpperCase(),answers.migrationsDir)
        if(retVal[0]){
