...
function ask(){
    ...
    .prompt([
        ...
        if(answers.typeOfObject=="package"){
          let dbChange=[];
          dbChange.push(answers.nameOfObject.replace(/[ #$.&*-]/g,'_').toUpperCase() + "." + answers.typeOfObject.toLowerCase() + ".body")
          dbChange.push(answers.nameOfObject.replace(/[ #$.&*-]/g,'_').toUpperCase() + "." + answers.typeOfObject.toLowerCase() + ".specification")

          console.log("------------------------------------------ Output ------------------------------------------\n")
          for(var i=0;i<dbChange.length;i++){
            const retVal= generate.execute(answers.typeOfMigration.toString(),answers.schemaname,answers.username,dbChange[i],answers.migrationsDir)
            if(retVal[0]){
                console.log(colors.green('File '+retVal[1]+' was created successfully\n'))
                console.log(colors.green("The changelog file, ../migrations/"+answers.schemaname+"/changelog/"+answers.schemaname+".xml has been successfully updated\n"))
                
            }
          }
          console.log("-------------------------------------------------------------------------------------------------\n")
          console.log("------------------------------------------ Please Note ------------------------------------------")
          console.log("This script creates two files for a package, feel free to alter if you decide otherwise")
        }
        else{
        ...
        console.log("This script assumes you dont use / to split statements, if you want to, add endDelimiter=\"\n/\s*\\n|\\n/\s*$\" and change splitStatements=\"true\" in your changelog file")
        console.log("-------------------------------------------------------------------------------------------------")
        ask()
