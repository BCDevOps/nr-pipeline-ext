...
class Jira {
    ...
    async _manageRfcTransitionToInitialState(rfcIssue) {
        ...
        if (rfcIssueStatus === RFCWORKFLOW.STATUS_OPEN_FOR_INT.name) {
        } else if (rfcIssueStatus !== RFCWORKFLOW.STATUS_CLOSED.name) {
            await this._transition(rfcIssueKey, RFCWORKFLOW.ACTION_201.name) // 201
            await this._transition(rfcIssueKey, RFCWORKFLOW.ACTION_81.name)
            // 81
        } else {
            await this._transition(rfcIssueKey, RFCWORKFLOW.ACTION_81.name) // 81
        }
    }
