...
class Jira {
   ...
   async createRFD() {
      ...
      const commitHash = this.jiraSettings.commitHash;
      await this.initializeProjectComponent(projectName, repoName);
      var containRfcToRfdLinks = 0
      //await self.editIssue(rfcIssueKey,commitHash)
      var rfdIssueLinks = []
      ...
      if (!containsAutomatedRfdIssueLinks) {
         ...
         for (let [jiraTargetEnv, reviewer] of REVIEWER_ENV_MAP) {
            ...
            const newRfdIssue = await self.createRfdAndLinkToRfc(rfcIssueKey, projectName, repoName, changeBranch, branchName, jiraTargetEnv, commitHash);
          ...
          for ( var i in rfcIssue.fields.issuelinks){
              if(rfcIssue.fields.issuelinks[i].type.name == "RFC-RFD" ){
                  ...
                  if (rfdIssueInfo.fields.labels.includes("auto")){
                   ...
                   //await self.editIssue(rfcIssue.fields.issuelinks[i].outwardIssue.key, commitHash)
                  }
               }
          }   
      }
    ...
    * @param {*} jiraTargetEnv set the "Target environment" on JIRA (this is custom field - customfield_10121)
    */
   async createRfdAndLinkToRfc(rfcIssueKey, projectName, repoName, changeBranch, branchName, jiraTargetEnv, commitHash) {
...
/* 
   ...
   async transitionRFDpostDeployment(env){
    ...
    async editIssue(issueKey, repoName){
      ...
      /* var commitId = null
      var noCommitId = 0
      for ( var i in issueInfo.fields.labels){
         if(issueInfo.fields.labels[i].includes('commit')){
            commitId = issueInfo.fields.labels[i].split(':')[1]
         }
         else {
            nocommitId+=1
         }
      }
      if (noCommitId == issueInfo.fields.labels.length || noCommitId == 0){
         var label = `[{set: commit:${commitHash}}]`
      }
      else {
         var label = `[{remove: commit:${commitId}}, {add: commit:${commitHash}}]`
      } 
      const editIssuePayload = {
         issueKey: issueKey,
         issue:{
