...
const RFDWOKFLOW = require('./JiraRfdWorkflowV1.2.2')
...
class Jira {
    ...
    async _transition(issueKey, transitionAction) {
        try {
            ...
            const foundTransition = possibleActions.find(t => t.name.toLowerCase() === transitionAction.toLowerCase())
            ...
            const e = new Error('Could not ' + transitionAction + ' for issueKey: ' + issueKey)
            // @ts-ignore
            e.original = error
            // prettier-ignore
    ...
    async _transitionRfdIssueToStatus(rfdIssue, finalStatus) {
        const RFD_LINEAR_FORWARD_WORKFLOW = [
            ...
            [RFDWOKFLOW.STATUS_SCHEDULED, RFDWOKFLOW.ACTION_4],
            [RFDWOKFLOW.STATUS_IN_PROGRESS, RFDWOKFLOW.ACTION_781],
            [RFDWOKFLOW.STATUS_RESOLVED, RFDWOKFLOW.ACTION_701],
        ]
