...
class Jira {
   ...
   async createRFD() {
     ...
     // const containsIssueLinks = rfcIssue.fields.issuelinks != undefined &&  rfcIssue.fields.issuelinks.length > 0;
      if (!containRfcToRfdLinks) {
         console.log("No RFC-RFD issueLinks found for RFC issue: " + rfcIssueKey);
         const changeBranch = this.jiraSettings.changeBranch;
         const branchName = this.jiraSettings.branchName;
         const self = this;
         console.log(containRfcToRfdLinks)
         ...
         for (let index = 0; index < rfcIssue.fields.issuelinks.length ; index ++) {
            if(issuelinks[`${index}`].type.name=="RFC-RFD"){
