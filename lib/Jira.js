...
class Jira {
    ...
    async createRFD() {
        ...
        if (!containsAutomatedRfdIssueLinks) {
            ...
            // loop for each env.
            for (const [jiraTargetEnv, reviewer] of REVIEWER_ENV_MAP) {
                // new RFD created.
                // await self.editIssue(rfcIssueKey,repoName)
                const newRfdIssue = await self.createRfdAndLinkToRfc(
                    rfcIssueKey,
                    projectName,
                    repoName,
                    changeBranch,
            ...
            for (const i in rfcIssue.fields.issuelinks) {
                if (rfcIssue.fields.issuelinks[i].type.name === 'RFC-RFD') {
                    const rfdIssueInfo = await this.getIssue(rfcIssue.fields.issuelinks[i].outwardIssue.key)
                    if (rfdIssueInfo.fields.labels.includes('auto')) {
                        await self._manageRfdAndSubtasksTransitionToInitialState(
                            rfcIssue.fields.issuelinks[i].outwardIssue.key
                        )
                    }
                }
            }
        ...
        try {
            ...
            if (foundTransition === undefined) {
                ...
                throw new Error(
                    "Could not '" +
                        transitionAction +
                        "' issue. Only these actions are possible: " +
                        possibleTreansitionActionName
                )
            }
            ...
            for (const i in rfcIssue.fields.issuelinks) {
                if (rfcIssue.fields.issuelinks[i].type.name === 'RFC-RFD') {
                    ...
                    if (
                        ...
                        rfcIssue.fields.issuelinks[i].outwardIssue.fields.summary.includes(jiraTargetEnv)
                    ) {
    ...
    async _manageRfdAndSubtasksTransitionToInitialState(rfdIssueKey) {
        ...
        } else if (rfdIssueStatus === ISSUE_STATUS_NAME.RESOLVED || rfdIssueStatus === ISSUE_STATUS_NAME.CLOSED) {
    ...
    transitionRFDpostDeployment(env) {
        // Jira settings
        ...
        this.transitionRfdAndSubtasksToResolved(rfcIssueKey, env)
    }

    transitionRfdAndSubtasksToResolved(rfcIssueKey, env) {
        if (env === CONST.ENV.DLVR) {
            this.transitionRfdAndSubtasksToResolvedOnTargetEnv(rfcIssueKey, CONST.JIRA_TARGET_ENV.DLVR)
        } else if (env === CONST.ENV.TEST) {
            this.transitionRfdAndSubtasksToResolvedOnTargetEnv(rfcIssueKey, CONST.JIRA_TARGET_ENV.TEST)
        } else if (env === CONST.ENV.PROD) {
            this.transitionRfdAndSubtasksToResolvedOnTargetEnv(rfcIssueKey, CONST.JIRA_TARGET_ENV.PROD)
        }
    }
    ...
    async transitionRfdAndSubtasksToResolvedOnTargetEnv(rfcIssueKey, jiraTargetEnv) {
        ...
        for (const rfdIssuekey of onTargetEnvRfdIssueKeys) {
            ...
            for (const subTask of rfdSubtaskInfo) {
