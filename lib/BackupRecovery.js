...
class BackupRecovery {
    ...
    async doBackupOrRecovery(op, schemaList) {
        ...
        const overrides = this._getOverride(backupOrRecoveryScriptSecretName, this.settings.tnsAdminSecretName, 
                                                this.settings.dbCredentialSecretName, op);
        const overrides_opt = "--overrides=" + JSON.stringify(overrides);

        return this._handleOcRunSqlclWithOverride(op, overrides_opt, backupOrRecoveryScriptSecretName)
    ...
    async createBackupRecoveryScriptSecret(options) {
        ...
        const bkryFilename_noExtension = this.obtainBackupRecoveryFileName_noExtension();
    ...
    obtainBackupRecoveryFileName_noExtension() {
        return `${this.settings.projectName}.${this.settings.changeBranch}.${this.settings.prNumber}`;
    }

    // The function was just a extraction for making testing easier.
    _handleOcRunSqlclWithOverride(op, overrides_opt, backupOrRecoveryScriptSecretName) {
        const namespace = this.settings.namespace;
    ...
    async _writeBackupRecoveryScript(op, backupRecoveryScriptFilePath, bkryFilename_noExtension, schemasList) {
        ...
        await writeToFile(backupRecoveryScriptFilePath, `           NULL; -- suppress ORA${ORA_CODE_CREATED_FILE_ALREADY_EXISTS} for dump file already exist\n`, append);
        await writeToFile(backupRecoveryScriptFilePath, '           -- We can have this in the \'future\' to raise our custom error to be able to handle in node.\n', append);
        await writeToFile(backupRecoveryScriptFilePath, '           -- RAISE_APPLICATION_ERROR(-20001, \'Warning! - File exists\');\n', append);  
     ...
     */
    _getOverride(backupOrRecoveryScriptSecretName, tnsAdminSecretName, dbCredentialSecretName, op) {
