...
module.exports = class GitOp {
    ...
    return new Promise((resolve, reject) => {
      ...
      child.on('exit', (exitCode) => {
        ...
        if (exitCode === 0) {
          ...
          reject(new Error({ stdout, stderr, exitCode, cwd: this.settings.dir }))
        }
      })
    })
  }
      ...
      .catch((proc) => {
        ...
        return Promise.resolve(true)
          .then(() => {
            ...
            fs.writeFileSync(this.tmpCredStoreFile, `https://${cred.username}:${cred.password}@${host}\n`, { encoding: 'utf8' })
          })
          .then(() => {
            return childProcess('git', ['config', '--local', 'credential.helper', gitCredentialHelper], { cwd: workDir })
          })
          .then(() => {
  ...
  isTargetBranchOutofSync () {
    return this._setupCredentials(this.settings.dir, this.settings.url, this.settings.credentials)
      .then(() => {
        return this.getMergeBase(this.settings.branch.name, this.settings.change.target)
          .then((commitHash1) => {
