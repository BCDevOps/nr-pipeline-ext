    ...
    gitOp.prepare()
      .then(async () => {
        for (let outputFile of outputfiles) {
          await childProcess('mv',[outputFile, '.'], {cwd: baseDir})
        }
      })
      .then(() => childProcess('git',['add', '.'], {cwd: baseDir}))
      .then(() => childProcess('git',['commit', '-m', "'New Scheam detail/linting information'"], {cwd: baseDir}))
      .then(() => childProcess('git',['pull', 'origin', changeBranch, '--rebase'], {cwd: baseDir}))
      .then(() => childProcess('git',['push', '--set-upstream', daReviewGitRepoUrl, changeBranch], {cwd: baseDir}))
      .catch((err) => {
        throw new Error(err)
      })
