...
module.exports = class {
   ...
   async isReadyForDeployment(env, rfcIssueKey) {
      ...
      let result = {status: null, rfcRfdContext, reason:{}}; // default return object;
   ...
   _printVerifiedStatus(verifiedResult) {
      ...
      if (reasonCodes && reasonCodes.includes(REASON.REASON_CODE_RFC_NOT_AUTHORIZED)) {
         ...
         const notAuthorizedRfc = verifiedResult.reason[REASON.REASON_CODE_RFD_BLOCKED].issueItems[0];
         console.group();
         console.log(actionRequiredTxtStyle(`RFC '${verifiedResult.rfcRfdContext.rfcIssueKey}' has not been authroized to '${env}'. Please authorize the RFC to '${env}' before deployment.`));
         console.groupEnd();
         console.log("\n");
      }
