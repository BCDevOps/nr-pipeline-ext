...
//const this.settings = require(`${process.cwd()}/lib/this.settings.js`)
      ...
      var sizeofMap=Array.from(mapRfdAutoBlockedIssue.keys())
       if(sizeofMap.length!=0){
           if(rfdAutoBlockStatus.every( (val)  => val === "Resolved")){
                ...
                var status=null;
                if(Array.from(mapRfdAutoIssue.values()).every( (val, arr) => val ==="Approved") && (((env.toLowerCase()==="dlvr"|| env.toLowerCase()==="int") && rfcStatus.toLowerCase()==="authorized for int")|| (env.toLowerCase()==="test" && rfcStatus.toLowerCase()==="authorized for test") || (env.toLowerCase()==="prod" && rfcStatus.toLowerCase()==="authorized for prod"))){
                   if(prevEnv){
                  ...
                  if(status=="Ready"){
                     ...
                     for ( var i of mapRfdAutoIssue.keys()){
                        ...
                        console.log("\n---------------------------     CURRENT RFD STATUS     ---------------------------\n")
                         if(mapRfdIssueStatus.get(i).toLowerCase()!="approved"){
                           ...
                           if(issueInfo.fields.labels.includes("auto")){
                             console.log("=> Status of Automated RFD:", i, "is currently ", mapRfdAutoIssue.get(i), "; ACTION REQUIRED: Approve to continue deploying to ", env.toUpperCase())
                           }
                           else {
                              console.log("=> Status of Manual RFD:", i, "is currently ", mapRfdAutoIssue.get(i), "; ACTION REQUIRED: Approve to continue deploying to ", env.toUpperCase() )
                           } 
                        }
                         else {
                           if(issueInfo.fields.labels.includes("auto")){
                     ...
                     if(prevEnv){
                       ...
                       for ( var i of mapRfdPrevEnvIssue.keys()){
                          ...
                          if(mapRfdPrevEnvIssue.get(i).toLowerCase()!="closed"){
                              ...
                              if(issueInfo.fields.labels.includes("auto")){
                                console.log("=> Status of Automated RFD:", i, "for env:", prevEnv, "is currently ", mapRfdPrevEnvIssue.get(i), "; ACTION REQUIRED: Close to continue deploying to ", env.toUpperCase())
                              }
                              else{
                                 console.log("=> Status of Manual RFD:", i,"for env:", prevEnv, "is currently ", mapRfdPrevEnvIssue.get(i), "; ACTION REQUIRED: Close to continue deploying to ", env.toUpperCase())
                              }
                          }
                         else {
                           if(issueInfo.fields.labels.includes("auto")){
                     ...
                     if(((env.toLowerCase()==="dlvr"|| env.toLowerCase()==="int") && rfcStatus.toLowerCase()==="authorized for int") || (env.toLowerCase()==="test" && rfcStatus.toLowerCase()==="authorized for test") || (env.toLowerCase()==="prod" && rfcStatus.toLowerCase()==="authorized for prod")){
                        ...
                        console.log("=> Current RFC Status is", rfcStatus, ", ACTION REQUIRED: Authorize RFC", rfcIssueKey, "to", env.toUpperCase() , "to continue deploying" )
                       }
                   }
         ...
         var rfcStatus = rfcInfo.fields.status.name
         var status=null;
         if(Array.from(mapRfdIssueStatus.values()).every( (val, arr) => val ==="Approved") && (((env.toLowerCase()==="dlvr"|| env.toLowerCase()==="int") && rfcStatus.toLowerCase()==="authorized for int")|| (env.toLowerCase()==="test" && rfcStatus.toLowerCase()==="authorized for test") || (env.toLowerCase()==="prod" && rfcStatus.toLowerCase()==="authorized for prod"))){
            if(prevEnv){
               if(Array.from(mapRfdPrevEnvIssue.values()).every( (val, arr) => val ==="Closed")){
                  status="Ready"
               }
         ...
         if(status==="Ready"){
            ...
            for ( var i of mapRfdIssueStatus.keys()){
               ...
               if(mapRfdIssueStatus.get(i).toLowerCase()!="approved"){
                  if(issueInfo.fields.labels.includes("auto")){
                     console.log("=> Status of Automated RFD:", i, "is currently ", mapRfdIssueStatus.get(i), "; ACTION REQUIRED: Approve to continue deploying to ", env.toUpperCase())
                  }
                  else {
                     console.log("=> Status of Manual RFD:", i, "is currently ", mapRfdIssueStatus.get(i), "; ACTION REQUIRED: Approve to continue deploying to ", env.toUpperCase() )
                  } 
               }
               else {
                     if(issueInfo.fields.labels.includes("auto")){
