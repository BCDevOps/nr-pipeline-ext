...
const { ENV, ISSUE_LINK_TYPE_NAME, VERIFY_STATUS, REASON } = require('./constants')
    ...
    async isReadyForDeployment(env, rfcIssueKey) {
        ...
        if (
            ((env === ENV.DLVR || env.toLowerCase() === 'int') &&
                rfcStatus.toLowerCase() !== RFCWORKFLOW.STATUS_AUTHORIZED_FOR_INT.name.toLowerCase()) ||
            (env === ENV.TEST &&
                rfcStatus.toLowerCase() !== RFCWORKFLOW.STATUS_AUTHORIZED_FOR_TEST.name.toLowerCase()) ||
            (env === ENV.PROD && rfcStatus.toLowerCase() !== RFCWORKFLOW.STATUS_AUTHORIZED_FOR_PROD.name.toLowerCase())
        ) {
