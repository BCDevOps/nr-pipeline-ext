...
module.exports = class {
    ...
    __earFolderShouldExist() {
        if (!this.__isDir(`${this.cwd}/ear`)) {
            if (this.__isDir(`${this.cwd}/src`)) {
                ...
                childProcess('mv', args, { encoding: 'utf-8' }).then(res => {
                ...
                return Promise.reject(new Error('src folder should exist in a java project'))
            }
        } else {
    ...
    async __initializePipelineFolder() {
        ...
        return new Promise((resolve, reject) => {
            rsync.execute(function(error) {
                ...
                resolve(self.__customizeConfigJS())
            })
        })
    }
    ...
    __customizeConfigJS() {
        ...
        return inquirer
            ...
            .catch(err => {
                throw new Error(`The error occured while customize config JS: ${err}`)
            })
    }
    ...
    __initializeOpenShiftFolder() {
        if (!this.__isDir(`${this.cwd}/openshift`)) {
            ...
            childProcess('cp', args, { encoding: 'utf-8' })
                ...
                .catch(err => {
                    throw new Error(`Could not copy openshift templates: ${err}`)
                })
        }
    }
