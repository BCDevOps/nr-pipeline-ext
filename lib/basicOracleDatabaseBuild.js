...
(async()=>{
    ...
    await _gitTargetSyncVerify(gitUrl, changeBranch, changeTarget, username, password);
})();
...
*/
async function _gitTargetSyncVerify(gitUrl, changeBranch, changeTarget, username, password) {
const git = new Git();
let result = await git.verify(changeBranch, changeTarget, username, password, gitUrl);
if(result == 'True') {
