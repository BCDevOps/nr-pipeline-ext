   ...
   createChangeset(typeOfMigration,dbChange,changesetPath,changelogPath,version){ 
        if(typeOfMigration.toLowerCase()=='versioned'){       
           let tDate=moment.tz(Date.now(),'America/Vancouver').format('YYYYMMDDHHmmss')
            ...
            if(result){
                  throw new Error(colors.red("This changeset file already exists! Please modify the same file for making changes and ensure it is a repeatable migration"))
            }
            else{
    ...
    updateChangelog(typeOfMigration,changelogPath,changeset,username,id){
        ...
        if(this.verifyifChangesetExists(changeset,changelogPath)){
            ...
            throw new Error(colors.red("This file already exists, meaning there is a changeset created for this object, modify the same file and make sure it is Repeatable Migration in the changelog file"));
        }
        else {
            if(typeOfMigration.toLowerCase()=='versioned') {
              ...
              fs.appendFileSync(changelogPath, startText, function(err) {
                  ...
                  var text="\t<sqlFile path=\"../sql/" +nameOfChangeSet +"\" splitStatements=\"false\" relativeToChangelogFile=\"true\"/>" + "\n"
              ...
              fs.appendFileSync(changelogPath, endText, function(err){
                  if(err) {
                      throw new Error(colors.red("File Cannot be Appended"))
                  }
              });
