...
module.exports = class {
  ...
  updateChangelog (typeOfMigration, changelogPath, changeset, username, id) {
    ...
    if (this.verifyifChangesetExists(changeset, changelogPath)) {
      ...
      const text = '\t<sqlFile path="../sql/' + nameOfChangeSet + '" splitStatements="false" relativeToChangelogFile="true"/>' + '\n'
    ...
    try {
      ...
      schemaFolders.forEach(item => {
        ...
        if (self.isDir(schemaDir) && item === schemaName) {
          ...
          const changesetName = self.createChangeset(typeOfMigration, dbChange, changesetPath, changelogPath, version)
