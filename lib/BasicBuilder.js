...
module.exports = class {
    ...
    async build() {
        ...
        const EMPTY = ''
        const changeTarget =
            settings.options.git.change !== undefined
                ? settings.options.git.change.target !== undefined
                    ? settings.options.git.change.target.trim()
                    : EMPTY
                : EMPTY
    ...
    async _createJiraAutoRFDs(jiraUrl, repoName, changeBranch, branchName, username, password) {
        const rfcIssueKey = Jira.parseJiraIssueKey(changeBranch)
