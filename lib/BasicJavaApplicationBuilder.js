...
module.exports = class {
  ...
  async build(){
    ...
    if(config.options.git.change){
       changeTarget = config.options.git.change.target
      ...
      // Create the jira object of the type Jira 
       ...
       return gitObj.verify(changeBranch,changeTarget).then(result=>{
        if(result=='True'){
          ...
          throw new Error("Your branch is out of sync from target, rebase, push and then run the Pipeline")
