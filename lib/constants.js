'use strict'
const ISSUE_TYPE_CODE = Freeze({ RFD: 'RFD', RFD_SUBTASK: 'RFD-subtask' })
const ISSUE_STATUS_NAME = Freeze({
    SUBMITTED: 'Submitted',
    CLOSED: 'Closed',
    REOPENED: 'Reopened',
    RESOLVED: 'Resolved',
...
const ISSUE_TRANSITION_ACTION_NAME = Freeze({
    REOPEN: 'Reopen',
...
const ISSUE_LINK_TYPE_NAME = Freeze({ RFC_FRD: 'RFC-RFD' })

const ENV = Freeze({ BUILD: 'build', DEV: 'dev', DLVR: 'dlvr', TEST: 'test', PROD: 'prod' })
const JIRA_TARGET_ENV = Freeze({ DLVR: 'DLVR', TEST: 'TEST', PROD: 'PROD' })
const REVIEWER_ENV_MAP = new Map([
    [JIRA_TARGET_ENV.DLVR, 'Developer'],
    [JIRA_TARGET_ENV.TEST, 'IIT'],
    [JIRA_TARGET_ENV.PROD, 'Business'],
])
const DB_ACTION = Freeze({ BACKUP: 'backup', RECOVERY: 'recovery' })

const VERIFY_STATUS = Freeze({ READY: 'Ready', NOT_READY: 'Not Ready' })
const REASON = Freeze({
